...
export class ComtReader {
    ...
    private static readonly PYRAMID_INDEX_LENGTH_START_INDEX = 12;

    private static readonly FRAGMENTS_INDEX_LENGTH_START_INDEX = 17;
    private static readonly INDEX_LENGTH_START_INDEX = 12;

    private static readonly NEW_METADATA_START_INDEX = 23;
    ...
    async readMetadata(): Promise<{ metadata: Metadata; metadataByteLength: number }> {
        ...
        await fs.read(fd, metadataLengthBuffer, 0, 4, ComtReader.NEW_METADATA_START_INDEX);
        const metadataLength = metadataLengthBuffer.readUInt32LE();

        const metadataBuffer = Buffer.alloc(metadataLength);
        await fs.read(fd, metadataBuffer, 0, metadataLength, ComtReader.NEW_METADATA_START_INDEX);
    ...
    async readPyramidIndex(
        metadataLength: number,
        tileOffsetBytes = ComtReader.SUPPORTED_TILE_OFFSET_BYTE_LENGTH,
    ): Promise<{pyramidEntries: number[], indexLength: number}> {
        if (tileOffsetBytes !== ComtReader.SUPPORTED_TILE_OFFSET_BYTE_LENGTH) {
            throw new Error(
                `Only a tile offset of ${ComtReader.SUPPORTED_TILE_OFFSET_BYTE_LENGTH} bytes is supported (yet).`,
            );
        ...
        const fd = await fs.open(this.fileName, "r");
        const indexLengthBuffer = Buffer.alloc(5);
        await fs.read(fd, indexLengthBuffer, 0, 5, ComtReader.PYRAMID_INDEX_LENGTH_START_INDEX);
        const indexLength = provider.convertUInt40LEToNumber(indexLengthBuffer.buffer, 0);

        const offset = ComtReader.NEW_METADATA_START_INDEX + metadataLength;
        const indexBuffer = Buffer.alloc(indexLength);
        await fs.read(fd, indexBuffer, 0, indexLength, offset);

        const pyramidIndexEntries: number[] = [];
        const numIndexEntries = indexBuffer.length / ComtReader.INDEX_ENTRY_BYTE_LENGTH;
        for (let i = 0; i < numIndexEntries; i++) {
            const size = indexBuffer.readUInt32LE();
            pyramidIndexEntries.push(size);
        }
        // decode and return values
        // entries are returning as empty during tests
        return { pyramidEntries: pyramidIndexEntries, indexLength };
    }

    async readFragmentsIndex(
        metadataLength: number,
        pyramidIndexLength: number,
        tileOffsetBytes = ComtReader.SUPPORTED_TILE_OFFSET_BYTE_LENGTH,
    ): Promise<{ fragmentEntries: number[], indexLength: number }> {
        if (tileOffsetBytes !== ComtReader.SUPPORTED_TILE_OFFSET_BYTE_LENGTH) {
            throw new Error(
                `Only a tile offset of ${ComtReader.SUPPORTED_TILE_OFFSET_BYTE_LENGTH} bytes is supported (yet).`,
            );
        ...
        const fd = await fs.open(this.fileName, "r");
        const fragmentsIndexLengthBuffer = Buffer.alloc(5);
        await fs.read(fd, fragmentsIndexLengthBuffer, 0, 5, ComtReader.FRAGMENTS_INDEX_LENGTH_START_INDEX);
        const indexLength = provider.convertUInt40LEToNumber(fragmentsIndexLengthBuffer.buffer, 0);

        const offset = pyramidIndexLength + ComtReader.NEW_METADATA_START_INDEX + metadataLength;
        const indexBuffer = Buffer.alloc(indexLength);
        await fs.read(fd, indexBuffer, 0, indexLength, offset);

        const fragmentsIndexEntries = [];
        const numIndexEntries = indexBuffer.length / ComtReader.INDEX_ENTRY_BYTE_LENGTH;
        for (let i = 0; i < numIndexEntries; i++) {
            const size = indexBuffer.readUInt32LE();
            fragmentsIndexEntries.push({ offset, size });
        }
        // decode and return values
        // entries are returning as empty during tests
        return { fragmentEntries: fragmentsIndexEntries, indexLength };
    }
